...
public class PreciseConvexHull {
	 ...
	 */
	public PreciseConvexHull(Geometry geometry) {
	 ...
	 */
	public PreciseConvexHull(Coordinate[] pts, GeometryFactory geomFactory) {
		inputPts = pts;
	...
	/**
	 * Returns a {@link Geometry} that represents the convex hull of the input geometry. The returned geometry contains the minimal number of points needed to represent the convex
	 * hull. In particular, no more than two consecutive points will be collinear.
	 *
	 * @return if the convex hull contains 3 or more points, a {@link Polygon}; 2 points, a {@link LineString}; 1 point, a {@link Point}; 0 points, an empty
	...
	public Geometry getConvexHull() {
		...
		// return lineOrPolygon(sortedPts);
	...
	/**
	 * Uses a heuristic to reduce the number of points scanned to compute the hull. The heuristic is to find a polygon guaranteed to be in (or on) the hull, and eliminate all
	 * points inside it. A quadrilateral defined by the extremal points in the four orthogonal directions can be used, but even more inclusive is to use an octilateral defined by
	...
	private Coordinate[] reduce(Coordinate[] inputPts) {
		...
		/**
		 * Add all unique points not in the interior poly. CGAlgorithms.isPointInRing is not defined for points actually on the ring, but this doesn't matter since the points of
	...
	private Coordinate[] preSort(Coordinate[] pts) {
		...
		Coordinate[] ptsTmp = pts;
		pts = new Coordinate[ptsTmp.length + 1];
		for (int i = 0; i < ptsTmp.length; i++) {
			pts[i] = ptsTmp[i];
		}
		pts[ptsTmp.length] = ptsTmp[0];
		// radialSort(pts);
		return pts;
	}
	...
	/*
	 * // MD - no longer used, but keep for reference purposes private Coordinate[] computeQuad(Coordinate[] inputPts) { BigQuad bigQuad = bigQuad(inputPts);
	 * 
	 * // Build a linear ring defining a big poly. ArrayList bigPoly = new ArrayList(); bigPoly.add(bigQuad.westmost); if (! bigPoly.contains(bigQuad.northmost)) {
	 * bigPoly.add(bigQuad.northmost); } if (! bigPoly.contains(bigQuad.eastmost)) { bigPoly.add(bigQuad.eastmost); } if (! bigPoly.contains(bigQuad.southmost)) {
	 * bigPoly.add(bigQuad.southmost); } // points must all lie in a line if (bigPoly.size() < 3) { return null; } // closing point bigPoly.add(bigQuad.westmost);
	 * 
	 * Coordinate[] bigPolyArray = CoordinateArrays.toCoordinateArray(bigPoly);
	 * 
	 * return bigPolyArray; }
	 * 
	 * private BigQuad bigQuad(Coordinate[] pts) { BigQuad bigQuad = new BigQuad(); bigQuad.northmost = pts[0]; bigQuad.southmost = pts[0]; bigQuad.westmost = pts[0];
	 * bigQuad.eastmost = pts[0]; for (int i = 1; i < pts.length; i++) { if (pts[i].x < bigQuad.westmost.x) { bigQuad.westmost = pts[i]; } if (pts[i].x > bigQuad.eastmost.x) {
	 * bigQuad.eastmost = pts[i]; } if (pts[i].y < bigQuad.southmost.y) { bigQuad.southmost = pts[i]; } if (pts[i].y > bigQuad.northmost.y) { bigQuad.northmost = pts[i]; } } return
	 * bigQuad; }
	 * 
	 * private static class BigQuad { public Coordinate northmost; public Coordinate southmost; public Coordinate westmost; public Coordinate eastmost; }
	 */

	/**
	 * @param vertices
	 *            the vertices of a linear ring, which may or may not be flattened (i.e. vertices collinear)
	 * @return a 2-vertex <code>LineString</code> if the vertices are collinear; otherwise, a <code>Polygon</code> with unnecessary (collinear) vertices removed
	 */
	...
	private static class RadialComparator implements Comparator {
		...
		/**
		 * Given two points p and q compare them with respect to their radial ordering about point o. First checks radial ordering. If points are collinear, the comparison is based
		...
		private static int polarCompare(Coordinate o, Coordinate p, Coordinate q) {
			...
			/*
			 * // MD - non-robust int result = 0; double alph = Math.atan2(dxp, dyp); double beta = Math.atan2(dxq, dyq); if (alph < beta) { result = -1; } if (alph > beta) {
