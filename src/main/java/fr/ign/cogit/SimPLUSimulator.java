...
public class SimPLUSimulator {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String rootParam = SimPLUSimulator.class.getClassLoader()
        .getResource("paramSet/scenar0MKDom/").getPath();
    ...
    SimPLUSimulator simplu = new SimPLUSimulator(rootFolder, geoFile, pluFile,
        selectedParcels, p.getString("listZipCode"), p, lF);
   ...
   * @throws Exception
   */
  public SimPLUSimulator(File rootfile, File geoFile, File pluFile,
      File selectedParcels, String zipcode, Parameters pa, List<File> lF)
      throws Exception {
	...
	if (rNU) {
		zoningFile = GetFromGeom.getPAU(pluFile, new File(p.getString("geoFile")), new File(rootfile, "tmp"), zipCode);
	} else {
    ...
    if (!(new File(simuFile.getParentFile(), "/snap/route.shp")).exists()) {
      System.out.println("in snapDatas"
          + GetFromGeom.getBati(new File(rootfile, "donneeGeographiques")));
      File snapFile = new File(simuFile.getParentFile(), "/snap/");
      snapFile.mkdir();

      buildFile = Vectors.snapDatas(
          GetFromGeom.getBati(new File(rootfile, "donneeGeographiques")),
          zoningFile, new File(simuFile.getParentFile(), "/snap/batiment.shp"));
      roadFile = Vectors.snapDatas(
          GetFromGeom.getRoute(new File(rootfile, "donneeGeographiques")),
          zoningFile, new File(simuFile.getParentFile(), "/snap/route.shp"));
      filePrescPonct = Vectors.snapDatas(
          new File(pluFile, "prescPonctRegroupe.shp"), zoningFile,
          new File(simuFile.getParentFile(), "/snap/prescPonctRegroupe.shp"));
      filePrescLin = Vectors.snapDatas(
          new File(pluFile, "prescLinRegroupe.shp"), zoningFile,
          new File(simuFile.getParentFile(), "/snap/prescLinRegroupe.shp"));
      filePrescSurf = Vectors.snapDatas(
          new File(pluFile, "prescSurfRegroupe.shp"), zoningFile,
      ...
      filePrescPonct = new File(simuFile.getParentFile(),
          "/snap/prescPonctRegroupe.shp");
      filePrescLin = new File(simuFile.getParentFile(),
          "/snap/prescLinRegroupe.shp");
      filePrescSurf = new File(simuFile.getParentFile(),
          "/snap/prescSurfRegroupe.shp");
    }
  ...
  public SimPLUSimulator(File rootfile, File geoFile, File pluFile,
      File selectedParcels, SimpleFeature feat, String zipcode, Parameters pa,
      List<File> lF) throws Exception {
  ...
  /*
   * public static List<File> run(File rootFile, File geoFile, File pluFile,
   * File parcelfiles, String zipcode, Parameters p) throws Exception {
   * SimPLUSimulator SPLUS = new SimPLUSimulator(rootFile, geoFile, pluFile,
   * parcelfiles, null, zipcode, p); return SPLUS.run(); }
   */
  ...
  public File runOneSim(int numParcel) throws Exception {
    ...
    Environnement env = LoaderSHP.load(simuFile, codeFile, zoningFile, featFile,
        roadFile, buildFile, filePrescPonct, filePrescLin, filePrescSurf, null);

    // Prescription setting
    IFeatureCollection<Prescription> prescriptions = env.getPrescriptions();
    IFeatureCollection<Prescription> prescriptionUse = PrescriptionPreparator
        .preparePrescription(prescriptions, p);
  ...
  public List<File> run() throws Exception {
    ...
    Environnement env = LoaderSHP.load(simuFile, codeFile, zoningFile,
        parcelsFile, roadFile, buildFile, filePrescPonct, filePrescLin,
        filePrescSurf, null);
    ...
    if (listBatiSimu.isEmpty()) {
      System.out.println(
          "&&&&&&&&&&&&&& Aucun bâtiment n'a été simulé pour la commune "
              + zipCode + " &&&&&&&&&&&&&&");
      System.exit(1);
    }
   ...
   * @throws Exception
   */
  public File runSimulation(Environnement env, int i, Parameters p,
    ...
    System.out.println(
        "Parcelle code : " + bPU.getCadastralParcels().get(0).getCode());
    ...
    if (!USE_DIFFERENT_REGULATION_FOR_ONE_PARCEL
        || bPU.getCadastralParcels().get(0).getSubParcels().size() < 2) {
    ...
    Double areaParcels = bPU.getCadastralParcels().stream()
    ...
    for (GraphVertex<Cuboid> v : cc.getGraph().vertexSet()) {
      ...
      AttributeManager.addAttribute(feat, "Rotation", v.getValue().orientation,
          "Double");
      AttributeManager.addAttribute(feat, "SurfaceBox", feat.getGeom().area(),
          "Double");
  ...
  private CommonPredicateArtiScales<Cuboid, GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> preparePredicateOneRegulationBySubParcel(
      BasicPropertyUnit bPU, Parameters p2,
      IFeatureCollection<Prescription> prescriptionUse, Environnement env)
      throws Exception {
  ...
  private static PredicateArtiScales<Cuboid, GraphConfiguration<Cuboid>, BirthDeathModification<Cuboid>> preparePredicateOneRegulation(
      BasicPropertyUnit bPU, Parameters p,
      IFeatureCollection<Prescription> prescriptionUse, Environnement env)
      throws Exception {
    ...
    System.out
        .println("Regulation code : " + sPBiggest.getUrbaZone().getLibelle());

    ArtiScalesRegulation regle = (ArtiScalesRegulation) sPBiggest.getUrbaZone()
        .getZoneRegulation();
   ...
   * @throws Exception
   */
  protected static int fillSelectedParcels(File rootFile, File geoFile,
      File pluFile, File selectedParcels, int missingHousingUnits,
      String zipcode, Parameters p, List<File> lF) throws Exception {
    // Itérateurs sur les parcelles où l'on peut construire
    ShapefileDataStore parcelDS = new ShapefileDataStore(
        selectedParcels.toURI().toURL());
    SimpleFeatureIterator iterator = parcelDS.getFeatureSource().getFeatures()
        .features();
    ...
    try {
      ...
      while (missingHousingUnits > 0 && iterator.hasNext()) {
        ...
        SimPLUSimulator simPLUsimu = new SimPLUSimulator(rootFile, geoFile,
            pluFile, selectedParcels, sinlgeParcel, zipcode, p, lF);
        // On lance la simulation
        File batiSimulatedFile = simPLUsimu
            .runOneSim((int) sinlgeParcel.getAttribute("num"));
        ...
        if (!iterator.hasNext()) {
          System.out.println(
              " STILL MISSING : " + missingHousingUnits + " HOUSING UNITS");
        }
      }
    } finally {
      iterator.close();
